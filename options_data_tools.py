...
def add_days_to_exp(data, trade_date_col='trade_date', exp_date_col='exp_date',
                    new_days_to_exp_col='days_to_exp'):
    """ Calculate number of days to expiration and add it to DataFrame
    ...
    :param new_days_to_exp_col: name for days to expiration column generated by this function
    :return: unindexed DataFrame that is copy of input plus days to expiration column
    """
    data_copy = data.copy()
    data_copy[new_days_to_exp_col] = \
        (data[exp_date_col] - data[trade_date_col]).map(lambda td: td.days)
    return data_copy


def add_rate(data, trade_date_col='trade_date', days_to_exp_col='days_to_exp',
             new_rate_col='rate'):
    ...
    data_indexed = data.set_index([trade_date_col, days_to_exp_col])
    unique_index = data_indexed.index.unique()
    trade_dates = unique_index.get_level_values(trade_date_col)
    days_to_exps = unique_index.get_level_values(days_to_exp_col)
    # Get corresponding unique set of Treasury rates
    unique_rates_list = [get_treasury_rate(tr_rates, trade_date, days_to_exp)
                         for trade_date, days_to_exp, in zip(trade_dates, days_to_exps)]
...
def add_forward(data, trade_date_col='trade_date', exp_date_col='exp_date', strike_col='strike',
    ...
    c_minus_p_min_df = (c_minus_p_min_df.join(calls[[days_to_exp_col, rate_col]], how='left')
